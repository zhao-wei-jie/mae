...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_seg_loss(self, lab, pred):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        mask: [N, L], 0 is keep, 1 is remove, 
        """
        # target = self.patchify(lab)
        ...
        loss = nn.functional.binary_cross_entropy_with_logits(pred,lab,weight=mask)
